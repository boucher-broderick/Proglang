...
start(Input_file_path, Part_a_output_file_path, Part_b_output_file_path) ->
    ...
    lists:foreach(
        fun(Partition) ->
            spawn(graph_stats, partA, [Partition, PidA])
        end,
        ParsedPartition
    ),
    ...
    PartBPids = lists:map(
        fun(Partition) ->
            spawn(graph_stats, partB, [Partition, PidB])
        end,
        ParsedPartition
    ),
    %% Share the list of all partB processes amongst themselves.
    lists:foreach(
        fun(Pid) ->
            Pid ! {children_pids, PartBPids}
        end,
        PartBPids
    ),
